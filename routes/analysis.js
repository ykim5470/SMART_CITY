...
const output = {
    ...
    // 각 페이지당 보여줄 목록 갯수 수정시 '10'을 전부 수정하고 routes/index.js에서 middleware 수정할 것. 
		await analysis_list.findAndCountAll({ limit: req.query.limit, offset: req.skip }).then((results) => {
			const itemCount = results.count; 
      const pageCount = Math.ceil(results.count / req.query.limit); 
      const currentPage = req.query.page
      const hasNext = (paginate.hasNextPages(req)(pageCount))
      const hasMore = hasNext ? `list?page=${currentPage + 1}&limit=10` : `list?page=${currentPage}&limit=10`
      const hasprev = (currentPage > 1) ? `list?page=${currentPage - 1}&limit=10` : `list?page=${currentPage}&limit=10`
			res.render("analysis/al_list", { anaList: results.rows, pageCount, itemCount, pages: paginate.getArrayPages(req)(10, pageCount, req.query.page), nextUrl : hasMore , prevUrl: hasprev});
		});
	},
