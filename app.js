...
const { my_scheduleJob, start_end_time_generator } = require("./public/js/helpers/api_scheduler");
...
io.on("connection", function (socket) {
	...
	socket.on("모델 스케쥴러 조작", (data) => {
		...
		model_list.findOne({ where: { md_id: md_id }, attributes: ["al_time", "sub_data", "date_look_up", "max_data_load"] }).then((res) => {
			model_input.findAll({ where: { md_id: md_id }, attributes: ["ip_param", "ip_value"] }).then((user_input_res) => {
				...
				const user_input_value_count = user_input_value.length; // 유저 인풋 TB

				const model_manage_str = JSON.stringify(res);
				const model_manage_value = JSON.parse(model_manage_str);

				const { al_time, sub_data, date_look_up, max_data_load } = model_manage_value; // 등록 페이지 데이터

				let date = new Date();
				let end_time = moment(date).format();
				let parsed_date_look_up = JSON.parse(date_look_up)
				let start_time = start_end_time_generator(parsed_date_look_up, end_time)
				...
				const raw_data_sub_get = async () => {
					...
					console.log(`http://203.253.128.184:18227/temporal/entities/${sub_data_list.slice(
						0,
						-1
